...
function main(){
   ...
   if (rental == undefined){
    document.querySelector('.fivedy').innerText = "Please complete the information above"; 

   }else{
    document.querySelector('.fivedy').innerText = rental; 
...
fetch(myJSonCuzmelFile)
    ...
    .then(function (myJSonlocalObject) {
        ...
        const towns = myJSonlocalObject['cuzmel'].filter((towns) => {
            if(towns.name==="Honda PCX150 Scooter") {
                return towns;
            }
        });
        console.log(towns);
